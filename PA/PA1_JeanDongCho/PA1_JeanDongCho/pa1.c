...
* Description: This program gets input from user and do the laws and equation for them.					*
*******************************************************************************************/
...
int main(void) 
{
	char plainchar, encchar;
	int  pa;
	
	double mass, acceleration, force, radius, height, syvolume, u, v,f, theta,tan_theta,vout,r1,r2,vin,x1,x2,y1,y2,distance,px,pz,py	;
	...
	scanf("%d%lf%lf", &pa, &px, &pz);
	py = pa / (pa % 2) - (63 / (-17)) + px * pz;
